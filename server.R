...
my.server <- function(input, output) {
  ...
  output$plot2 <- renderPlot({
    ...
    ggplot(plot_data, aes((x = imdb_score), y = (gross), label=movie_title)) +
      ...
      theme(plot.title = element_text(face="bold", size=30, color = "coral")) +
      theme(axis.title = element_text(face="bold", size=20, color = "darkseagreen")) +
      theme(panel.background = element_rect(fill = "antiquewhite",colour = "antiquewhite", size = 1, linetype = "solid"))
  })
}
