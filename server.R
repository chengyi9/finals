...
my.server <- function(input, output) {
  ...
  output$table <- DT::renderDT({
    datatable((select(filter_data(), "movie_title", "plot_keywords", "duration", "imdb_score")), options = list(paging = TRUE), rownames = FALSE)
  })
  ...
  output$plot2 <- renderPlotly({
    ...
    ggplot(plot_data, aes(x = imdb_score, y = gross)) +
      labs(y = "Revenue Generated by a Movie", x = "IMDB Score") +
      geom_point(aes(label=movie_title)) +
